...
def convert_StringProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.StringProperty``."""
    if prop.multiline:
        kwargs['validators'].append(validators.length(max=500))
...
def convert_DateProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.DateProperty``."""
    if prop.auto_now or prop.auto_now_add:
        return None
...
def convert_TimeProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.TimeProperty``."""
    if prop.auto_now or prop.auto_now_add:
        return None
...
def convert_ListProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.ListProperty``."""
    return None


def convert_StringListProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.StringListProperty``."""
    return StringListPropertyField(**kwargs)


def convert_ReferenceProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.ReferenceProperty``."""
...
def convert_SelfReferenceProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.SelfReferenceProperty``."""
    return None


def convert_UserProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.UserProperty``."""
    return None


def convert_BlobProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.BlobProperty``."""
    return f.FileField(**kwargs)


def convert_TextProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.TextProperty``."""
    return f.TextAreaField(**kwargs)


def convert_CategoryProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.CategoryProperty``."""
    return get_TextField(kwargs)


def convert_LinkProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.LinkProperty``."""
    kwargs['validators'].append(validators.url())
    return get_TextField(kwargs)


def convert_EmailProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.EmailProperty``."""
    kwargs['validators'].append(validators.email())
    return get_TextField(kwargs)


def convert_GeoPtProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.GeoPtProperty``."""
    return GeoPtPropertyField(**kwargs)


def convert_IMProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.IMProperty``."""
    return None


def convert_PhoneNumberProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.PhoneNumberProperty``."""
    return get_TextField(kwargs)


def convert_PostalAddressProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.PostalAddressProperty``."""
    return get_TextField(kwargs)


def convert_RatingProperty(model, prop, kwargs):
    """Returns a forms field for a ``db.RatingProperty``."""
...
class ModelConverter(object):
    """
    Converts properties from a ``db.Model`` class to forms fields.
    ...
    def convert(self, model, prop, field_args):
        """
        Returns a forms field for a single model property.
...
def model_fields(model, only=None, exclude=None, field_args=None,
                 converter=None):
    """
    Extracts and returns a dictionary of forms fields for a given
    ``db.Model`` class.
    ...
    :param only:
        ...
        the forms. Only these properties will have fields.
    :param exclude:
        An optional iterable with the property names that should be excluded
        from the forms. All other properties will have fields.
    :param field_args:
...
def model_form(model, base_class=Form, only=None, exclude=None, field_args=None,
    ...
    ``db.Model`` class. The forms class can be used as it is or serve as a base
    for extended forms classes, which can then mix non-model related fields,
    subforms with other model forms, among other possibilities.

    :param model:
        The ``db.Model`` class to generate a forms for.
    :param base_class:
        Base forms class to extend from. Must be a ``wtforms.Form`` subclass.
    :param only:
        An optional iterable with the property names that should be included in
        the forms. Only these properties will have fields.
    :param exclude:
    ...
    # Return a dynamically created forms class, extending from base_class and
