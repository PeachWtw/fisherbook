...
def do_form_field(parser, token):
    """
    Render a WTForms forms field allowing optional HTML attributes.
    Invocation looks like this:
      {% form_field forms.username class="big_text" onclick="alert('hello')" %}
    where forms.username is the path to the field value we want.  Any number
    of key="value" arguments are supported. Unquoted values are resolved as
    template variables.
    """
    parts = token.contents.split(' ', 2)
    if len(parts) < 2:
        error_text = '%r tag must have the forms field name as the first value, followed by optional key="value" attributes.'
