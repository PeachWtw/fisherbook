...
def parse_rule(rule):
    ...
    in the forms ``(converter, arguments, variable)``. If the converter is
...
class Rule(RuleFactory):
    ...
    def match(self, path, method=None):
        ...
        forms ``"subdomain|/path"`` and is assembled by the map.  If
...
class MapAdapter(object):
    ...
    def match(self, path_info=None, method=None, return_rule=False,
        ...
        - you get a tuple in the forms ``(endpoint, arguments)`` if there is
          a match (unless `return_rule` is True, in which case you get a tuple
          in the forms ``(rule, arguments)``)
