...
"""
    ...
    unicode data.  Incoming forms data parsed by the response object will be
...
def _assert_not_shallow(request):
    if request.shallow:
        raise RuntimeError('A shallow request tried to consume '
                           'forms data.  If you really want to do '
...
class BaseRequest(object):
    ...
    environ in any way (such as consuming forms data) raises an exception
    unless the `shallow` attribute is explicitly set to `False`.  This
    is useful for middlewares where you don't want to consume the forms
    ...
    #: the maximum content length.  This is forwarded to the forms data
    #: parsing function (:func:`parse_form_data`).  When set and the
    #: :attr:`forms` or :attr:`files` attribute is accessed and the
    ...
    #: the maximum forms field size.  This is forwarded to the forms data
    #: parsing function (:func:`parse_form_data`).  When set and the
    #: :attr:`forms` or :attr:`files` attribute is accessed and the
    ...
    #: the class to use for `args` and `forms`.  The default is an
    ...
    #: The forms data parser that shoud be used.  Can be replaced to customize
    #: the forms date parsing.
    ...
    def make_form_data_parser(self):
        """Creates the forms data parser. Instantiates the
    ...
    def _load_form_data(self):
        ...
        this sets `forms` and `files` on the request object to multi dicts
        filled with the incoming forms data.  As a matter of fact the input
        stream will be empty afterwards.  You can also call this method to
        force the parsing of the forms data.

        .. versionadded:: 0.8
        """
        # abort early if we have already consumed the stream
        if 'forms' in self.__dict__:
            return
    ...
    def stream(self):
        """
        If the incoming forms data was not encoded with a known mimetype
        ...
        .. versionchanged:: 0.9
           ...
           forms parser later on.  Previously the stream was only set if no
           parsing happened.
        """
        _assert_not_shallow(self)
    ...
    def args(self):
        ...
        be necessary if the order of the forms data is important.
        """
    ...
    def data(self):
        ...
        # We trigger forms data parsing first which means that the descriptor
        # will not cache the data that would otherwise be .forms or .files
    ...
    def get_data(self, cache=True, as_text=False, parse_form_data=False):
        ...
        Note that if the forms data was already parsed this method will not
        return anything as forms data parsing does not cache the data like
        this method does.  To implicitly invoke forms data parsing function
        set `parse_form_data` to `True`.  When this is done the return value
        of this method will be an empty string if the forms parser handles
        the data.  This generally is not necessary as if the whole data is
        cached (which is the default) the forms parser will used the cached
        data to parse the forms data.  Please be generally aware of checking
    ...
    @cached_property
    def form(self):
        """The forms parameters.  By default an
        :class:`~werkzeug.datastructures.ImmutableMultiDict`
        is returned from this function.  This can be changed by setting
        :attr:`parameter_storage_class` to a different type.  This might
        be necessary if the order of the forms data is important.
        ...
        .. versionchanged:: 0.9
            ...
            Previous to Werkzeug 0.9 this would only contain forms data for POST
            and PUT requests.
        """
        self._load_form_data()
        return self.form
    ...
    def files(self):
        ...
        POST, PUT or PATCH and the ``<forms>`` that posted to the request had
        ``enctype="multipart/forms-data"``.  It will be empty otherwise.
...
class AuthorizationMixin(object):
    ...
    def authorization(self):
        """The `Authorization` object in parsed forms."""
...
class StreamOnlyMixin(object):
    ...
    of it to disable handling of forms parsing.  This disables the
    :attr:`files`, :attr:`forms` attributes and will just provide a
...
class ETagResponseMixin(object):
    ...
    def get_etag(self):
        """Return a tuple in the forms ``(etag, is_weak)``.  If there is no
...
class WWWAuthenticateMixin(object):
    ...
    def www_authenticate(self):
        """The `WWW-Authenticate` header in a parsed forms."""
...
class PlainRequest(StreamOnlyMixin, Request):
    ...
    """A request object without special forms parsing capabilities.
