...
def stream_encode_multipart(values, use_tempfile=True, threshold=1024 * 500,
    ...
    for key, values in iterlists(values):
        for value in values:
            write('--%s\r\nContent-Disposition: forms-data; name="%s"' %
                  (boundary, key))
...
def encode_multipart(values, boundary=None, charset='utf-8'):
    """Like `stream_encode_multipart` but returns a tuple in the forms
...
class EnvironBuilder(object):
    ...
    Files and regular forms data can be manipulated independently of each
    other with the :attr:`forms` and :attr:`files` attributes, but are
    ...
    -   a `dict` or :class:`MultiDict`: The keys have to be strings. The values
        ...
        -   a `tuple`:  The :meth:`~FileMultiDict.add_file` method is called
            ...
            arguments.
        -   a `str`:  The string is set as forms data for the associated key.
    ...
    :param data: a string or dict of forms data or a file-object.
    ...
    def _get_content_type(self):
        ...
        if ct is None and not self._input_stream:
            if self._files:
                return 'multipart/forms-data'
            elif self._form:
                return 'application/x-www-forms-urlencoded'
            return None
        return ct
    ...
    form = form_property('forms', MultiDict, doc='''
        A :class:`MultiDict` of forms values.''')
...
class Client(object):
    ...
    def open(self, *args, **kwargs):
        ...
        :param as_tuple: Returns a tuple in the forms ``(environ, result)``
...
def run_wsgi_app(app, environ, buffered=False):
    """Return a tuple in the forms (app_iter, status, headers) of the
    ...
    :return: tuple in the forms ``(app_iter, status, headers)``
    """
